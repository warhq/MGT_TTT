...
- **`worldMap.py`** — Generates an icosahedral world surface map for a UWP. `generate_world_map(uwp, out_path=None, seed=None) -> PIL.Image.Image`. Renders the unfolded 20-triangle icosahedron in the **World Builder's Handbook 5+10+5 net**: 5 point-up north-cap triangles (apices = north pole), a 10-triangle equatorial zigzag band, and 5 point-down south-cap triangles (apices = south pole). South caps are x-offset by `face_side/2` — that's the antiprism twist between the north and south pentagons, not a bug. Each face is subdivided into N² small terrain cells (N scales with UWP Size: 4/6/8/10), and cell terrain is chosen via fractal value noise seeded by the UWP. Hydrographics sets the water-area fraction (threshold picked to match exactly, via sorted-noise indexing). Atmosphere selects the palette (breathable / dusty / vacuum / exotic / corrosive). Polar ice is concentrated near the cap apex points by a `lat_factor > 0.75` threshold (cells near `y=0` or `y=3h` of the 3h-tall map). A "big hex" labelled with km-per-cell-edge, color swatches, and the world's UWP stats are drawn below the map. Same UWP always yields the same image (`seed` defaults to a SHA-256 hash of the UWP). Runnable: `python worldMap.py <UWP> [out.png]`.
